...
async def test_vision_rpc_handler():
    # 1. Setup mocks
    mock_node = MagicMock()
    mock_node.analyze_vision = AsyncMock(
        return_value={"make": "Tesla", "model": "Model 3"}
    )
    ...
    # 3. Assertions
    mock_node.analyze_vision.assert_called_once_with(
        [b"fake-image-bytes"], "Identify this car"
    )
