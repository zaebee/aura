...
appears, it refers to AP2 v0.2 (2026-04-28, now standardized under the FIDO
...
| Credential Provider | **Absent.** Buyers pay directly (SOL/USDC + memo); no credential issuance or per-use authorization | `core/src/aura_hive/hive/proteins/transaction/solana_engine.py:215` (`generate_payment_request`) |
| Merchant Payment Processor | Solana / EVM rails behind the Hive treasury (`EVMProvider`, `SolanaProvider`) | `core/src/aura_hive/hive/proteins/transaction/engine.py:70`, `core/src/aura_hive/hive/proteins/transaction/solana_engine.py:28` |
...
| Verification/processing logic must be deterministic code | The guard engine and the receipt verifier are pure, deterministic, keyless | `core/src/aura_hive/hive/proteins/guard/engine.py`, `core/src/aura_hive/hive/membrane/receipt.py:377` (`verify()`) |
...
- `max_x402_payment = 5.0` USDC cap on data purchases (`core/src/aura_hive/config/policy.py:13`) —
  a fixed platform constant, not per-agent delegation;
- `ui_trigger_price = 1000.0` (`core/src/aura_hive/config/policy.py:12`) — routes high-value bids
...
tolerance (`core/src/aura_hive/hive/proteins/transaction/solana_engine.py:158`). There is no checkout object, no signed
...
The gateway accepts ±60 s (`api-gateway/src/api_gateway/security.py:22,107`); receipts use `issued_at`
...
with Ed25519 (`tools/simulators/agent_identity.py:70-106`). That is fine while Aura is only the
...
The MCP server hardcodes the identity `"mcp-agent"`
(`synapses/mcp-server/src/aura_mcp/translator.py:39`) and embeds a full
`HiveCell` in-process (`synapses/mcp-server/src/aura_mcp/main.py:40-47`). Its
README describes a signed-gateway design the code does not
...
1. **Checkout object with hash binding.** Today a deal is a `LockedDeal`
   (`core/src/aura_hive/hive/proteins/persistence/engine.py:59`) + memo. A signed, hash-bound checkout is the
...
5. **x402 already exists.** HTTP 402 + `X-Payment-Instructions` with a 5 USDC
   cap (`core/src/aura_hive/hive/proteins/blockchain_data/skill.py:104-138`) — and x402 is one of AP2's supported
...
| Payment request URI builder | `core/src/aura_hive/hive/proteins/transaction/solana_engine.py:215` |
...
| Receipt signed content fields | `core/src/aura_hive/hive/membrane/receipt.py:197-222` |
