...
event = AuditEvent(timestamp=now, violations=[...])
...
- ✅ Backward compatibility via Proto field numbers
...
**Note:** Schema shown as JSON for readability. Actual wire format is binary Protobuf via `.SerializeToString()`.
...
from aura_core.gen.dna_pb2 import InjuryEvent
...
await nc.publish("aura.hive.injury", injury.SerializeToString())
```
...
from aura_core.gen.dna_pb2 import BrainDeadEvent

event = BrainDeadEvent(
    timestamp=time.time(),
    error=str(e),
    context={"request_id": "req_xyz789", "item_id": "widget_456"},
    fallback_action="counter_offer",
    fallback_price=55.0
)
await nc.publish("aura.core.brain_dead", event.SerializeToString())
```
...
from aura_core.gen.dna_pb2 import HeartbeatEvent

event = HeartbeatEvent(status="active", timestamp=time.time(), service="core")
await self.nc.publish("aura.hive.heartbeat", event.SerializeToString())
```
...
try:
    event = AuditEvent(timestamp=time.time(), violations=[...])
    await self.nc.publish("aura.hive.audit", event.SerializeToString())
except (nats.errors.ConnectionClosedError, nats.errors.TimeoutError) as e:
    logger.warning("nats_publish_failed", topic="aura.hive.audit", error=str(e))
```
...
async def message_handler(msg):
    ...
    logger.info("received_event", topic=msg.subject, violations=len(event.violations))
