    ...
    def validate_decision(self, decision: dict, context: dict) -> bool:
        ...
        if offered_price > 0:
            ...
            if margin < settings.safety.min_profit_margin:
                logger.warning("safety_margin_violation", offered_price=offered_price, internal_cost=internal_cost, margin=margin, min_margin=settings.safety.min_profit_margin)
        ...
        if action in ["accept", "counter"] and offered_price < floor_price:
            logger.warning("safety_floor_violation", action=action, offered_price=offered_price, floor_price=floor_price)
...
class HiveMembrane:
    ...
    async def inspect_inbound(self, signal: Any) -> Any:
        ...
        fields_to_scan = []
        if hasattr(signal, "item_id"): fields_to_scan.append(("item_id", signal.item_id))
        if hasattr(signal, "agent") and hasattr(signal.agent, "did"): fields_to_scan.append(("agent.did", signal.agent.did))
            ...
            if isinstance(value, str):
                ...
                for pattern in injection_patterns:
                    if pattern in lowered_val:
                        logger.warning("membrane_inbound_injection_detected", field=field_name, pattern=pattern)
                        if field_name == "item_id": signal.item_id = "INVALID_ID_POTENTIAL_INJECTION"
                        elif field_name == "agent.did": signal.agent.did = "REDACTED"
        return signal

    async def inspect_outbound(self, decision: IntentAction, context: HiveContext) -> IntentAction:
        floor_price = context.item_data.get("floor_price", 0.0)
        if isinstance(decision, FailureIntent) or decision.action == "error":
            return self._override_with_safe_offer(decision, floor_price * 1.05, "FAILURE_RECOVERY")
        ...
        if decision.action not in ["accept", "counter"]: return decision
        if decision.price < floor_price: return self._override_with_safe_offer(decision, floor_price * 1.05, "FLOOR_PRICE_VIOLATION")

        min_margin = getattr(self.settings.logic, "min_margin", DEFAULT_MIN_MARGIN)
        required_min_price = floor_price / (1 - min_margin)
        if decision.price < required_min_price: return self._override_with_safe_offer(decision, required_min_price, "MIN_MARGIN_VIOLATION")
        return decision
