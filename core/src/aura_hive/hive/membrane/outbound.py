...
from .verdict import _OVERRIDE, _REFUSE, _UNAVAILABLE, _Verdict
...
class Attestation:
    ...
    def __init__(self, registry: SkillRegistry | None, settings: Any = None) -> None:
...
class Substitution:
    ...
    async def offer(
        ...
        if not await self.postcondition.verify(rounded_price, guard_context, verdict):
        ...
        if rounded_price == orig_price and original.action in (
            ...
            return await self.finish(claim or original, original, verdict, request_id)
    ...
    async def finish(
        ...
        emission.receipt = await self.attestation.sign(receipt)
        ...
        **No decision the Intent carries is modified in place.** Every path that
        ...
        if params_name == "trade" and params_value is not None:
            ...
            risk_threshold = getattr(
                self.substitution.settings.safety, "trade_risk_threshold", 0.10
            )
