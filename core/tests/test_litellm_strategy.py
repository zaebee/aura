...
from src.hive.proteins.reasoning.enzymes.dspy_strategy import (
    AI_Decision,
    LiteLLMStrategy,
)
...
class TestLiteLLMStrategy:
    ...
    @patch("src.hive.proteins.reasoning.enzymes.dspy_strategy.LLMEngine")
    def test_evaluate_accept(self, mock_engine_class, mock_item):
        ...
        with (
            patch("src.hive.proteins.reasoning.enzymes.dspy_strategy.Template"),
            patch(
                "src.hive.proteins.reasoning.enzymes.dspy_strategy.open", create=True
            ),
        ):
            strategy = LiteLLMStrategy(model="gpt-3.5-turbo")
