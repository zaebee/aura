...
class TestLiteLLMStrategy:
    ...
    @patch("src.hive.proteins.reasoning.reasoning_engine.LLMEngine")
    def test_evaluate_accept(self, mock_engine_class, mock_item):
        ...
        with (
            patch("src.hive.proteins.reasoning.reasoning_engine.Template"),
            patch(
                "src.hive.proteins.reasoning.reasoning_engine.open", create=True
            ),
        ):
            strategy = LiteLLMStrategy(model="gpt-3.5-turbo")
